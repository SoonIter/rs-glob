...
switch (platform) {
    ...
    switch (arch) {
      case 'arm64':
        localFileExisted = existsSync(
          join(__dirname, 'rs-glob.android-arm64.node'),
        );
        try {
          if (localFileExisted)
      ...
      case 'arm':
        localFileExisted = existsSync(
          join(__dirname, 'rs-glob.android-arm-eabi.node'),
        );
        try {
          if (localFileExisted)
  ...
  case 'darwin':
    localFileExisted = existsSync(
      join(__dirname, 'rs-glob.darwin-universal.node'),
    );
    try {
      if (localFileExisted)
    ...
    switch (arch) {
      case 'x64':
        localFileExisted = existsSync(
          join(__dirname, 'rs-glob.darwin-x64.node'),
        );
        try {
          if (localFileExisted)
...
const { glob, rsGlob, hhh } = nativeBinding;

module.exports.glob = glob;
module.exports.rsGlob = rsGlob;
module.exports.hhh = hhh;
